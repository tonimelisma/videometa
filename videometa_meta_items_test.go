...
func TestDecodeEXIFFromUUIDBox(t *testing.T) {
	...
	c.Assert(flattenSourceTags(tags.EXIF())["Make"].Value, qt.Equals, "UUID Cam")
}
...
func TestDecodeEXIFFromMetaIlocFileOffset(t *testing.T) {
	...
	c.Assert(err, qt.IsNil)
	c.Assert(flattenSourceTags(tags.EXIF())["Make"].Value, qt.Equals, "Item Offset Cam")
}
...
func TestDecodeEXIFFromMetaIlocIDAT(t *testing.T) {
	...
	c.Assert(err, qt.IsNil)
	c.Assert(flattenSourceTags(tags.EXIF())["Model"].Value, qt.Equals, "IDAT Model")
}
...
func TestDecodeXMPFromMetaIlocIDAT(t *testing.T) {
	...
	c.Assert(err, qt.IsNil)
	c.Assert(flattenSourceTags(tags.XMP())["CreatorTool"].Value, qt.Equals, "Item XMP")
}
